...
TEST_CASE("Parse the most basic of CSV buffers with ', ' delimiter using initial space" *
          test_suite("Reader")) {
  Reader<delimiter<','>, quote_character<'"'>, first_row_is_header<false>, trim_policy::no_trimming>
      csv;
...
TEST_CASE("Parse CSV with missing columns" * test_suite("Reader")) {
  ...
  const std::vector<std::string> expected_cells{"a", "b", "c", "d", "1", "2",
                                                "",  "4", "5", "6", "",  "8"};
...
TEST_CASE("Parse the most basic of CSV buffers with double quotes with embedded delimiters" *
  ...
  const std::vector<std::string> expected_cells{"a",         "\"b,c\"",   "d",
                                                "\"1,2,3\"", "\"4,5,6\"", "\"7,8,9\""};
