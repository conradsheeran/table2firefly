...
class BillClassifier:
    ...
    def __init__(self, api_key: str):
        ...
        """
        if not api_key:
            raise ValueError("错误：必须提供 DeepSeek API Key。")
        try:
            self.client = OpenAI(api_key=api_key, base_url="https://api.deepseek.com")
        except Exception as e:
            raise Exception(f"初始化 API 客户端时出错: {e}")

    def _get_category_from_deepseek(self, merchant: str, description: str) -> str:
        """
        调用 DeepSeek API 对交易进行分类。
        """
        prompt = f"""
        你是一个智能账单分类助手。
    ...
    def _classify_transaction(self, row: pd.Series) -> str:
        ...
        # AI 分类
        return self._get_category_from_deepseek(merchant, description)
...
#
#     input_file = 'bill.csv'
#     output_file = 'processed_bill.csv'
#
